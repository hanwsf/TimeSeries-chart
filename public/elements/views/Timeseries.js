...
var lineData=[];
var pxsimplechart=document.querySelector("#realtimechart4"); // 获取simple-line-chart
																// 对象
// add for vis-chart
var pxtimeseriesvis=document.querySelector("#timeseriesvis"); 

var tagNames = new Array();
// millisPerPixel = 900 means 15 minutes, 1800 means 30 minutes, 3600 means 1
// hour
// var smoothie = new
// SmoothieChart({millisPerPixel:900,labels:{fillStyle:'#00ff00'},timestampFormatter:SmoothieChart.timeFormatter});
...
// securepag-view submit button calls onclick_machineServiceData() ->
// getTagsSelectedValue() and updateAllCharts, calls function updateChart (num)
...
/**
 * Method to update the Chart with the latest data from the selected tags This
 * method quries UAA and Timeseries directly
 */
...
function updateChart (num) {
    ...
    uaaRequest.onreadystatechange = function() {
      if (uaaRequest.readyState == 4) {
        ...
        // curl
		// 'https://time-series-store-predix.run.aws-jp01-pr.ice.predix.io/v1/datapoints/latest'
		// -X post -H 'predix-zone-id: cbe8918f-3052-4454-9ed0-4ced7d932b68' -H
		// 'authorization: Bearer Please log in first.' -H 'content-type:
...
function updateVisChart (num) {
    ...
    uaaRequest.onreadystatechange = function() {
      ...
      if (uaaRequest.readyState == 4) {  
        ...
        // can use peroid selector for start and end, refer to dashboards-view,
        ...
        // curl
		// 'https://time-series-store-predix.run.aws-jp01-pr.ice.predix.io/v1/datapoints'
		// -X post -H 'predix-zone-id: cbe8918f-3052-4454-9ed0-4ced7d932b68' -H
		// 'authorization: Bearer Please log in first.' -H 'content-type:
		// application/json' --data-binary
		// '{"cache_time":0,"tags":[{"name":"CompressionRatio","order":"desc"}],"start":1483273203000,"end":1485778803000}'
        // add for vis-chart
...
// if (vistimeSeriesGetData.status >= 200 && vistimeSeriesGetData.status < 400)
        	if (vistimeSeriesGetData.status == 400){
        		alert ("Data points are more than 20k, please narrow the period");
        		visfreshed = 0;
        	};
            if (vistimeSeriesGetData.status >= 200&& vistimeSeriesGetData.status < 400) {
...
/**
 * Method to make the necessary rest call and get the configurations from the
 * server
 */
